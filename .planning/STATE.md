...
last_updated: "2026-03-26T04:36:15.369Z"
progress:
  total_phases: 7
  completed_phases: 3
  total_plans: 26
...
**Current focus:** Phase 06 — main-path-personalization-and-content-credibility

## Current Position

Phase: 06 (main-path-personalization-and-content-credibility) — EXECUTING
Plan: 3 of 11
...
| Phase 05 P04 | 12min | 1 tasks | 2 files |
| Phase 05 P03 | 13min | 2 tasks | 9 files |
| Phase 05 P05 | 21min | 5 tasks | 7 files |
| Phase 05 P06 | 12min | 3 tasks | 9 files |
| Phase 06 P01 | 6min | 4 tasks | 5 files |
| Phase 06 P02 | 14m 20s | 5 tasks | 6 files |
...
- [Phase 05]: Derived onboardingCompleted from completed OnboardingRound count so login and /auth/me share one backend-owned truth.
- [Phase 05]: Kept LoginResult as the single auth response contract and aligned OAuth return types to it instead of introducing a parallel payload shape.
- [Phase 05]: Use user.onboardingCompleted as the single source of truth for post-auth onboarding routing instead of isNewUser.
- [Phase 05]: Keep onboarding API access inside dedicated React Query hooks with stable array query keys and mutation invalidation.
- [Phase 05]: Round renderers keep answer state local and emit typed payloads upward so the onboarding container can own server mutations only.
- [Phase 05]: RecommendationPanel confirms with learningPathId from the backend recommendation payload instead of resolving slugs on the frontend.
- [Phase 05]: Onboarding.tsx now derives active round, resume visibility, recommendation state, and redirect behavior from React Query hooks instead of direct axios calls.
- [Phase 05]: Locked onboarding copy is centralized in vi.ts and reused by child components to keep resume and confirm flows consistent with the UI-SPEC contract.
- [Phase 06]: Store the prioritized main path on LearnerProfile as a nullable foreign key so secondary enrollments remain available while one canonical journey is prioritized.
- [Phase 06]: Seed mainLearningPathId only when null so the first confirmed path becomes the main path and later enrollments never overwrite it automatically.
- [Phase 06]: Keep PendingOnboardingRound lightweight with userId-only persistence because later onboarding rounds only need queryable pending state, not full relational ownership.
- [Phase 06]: Use a shared rankings array contract across prompt, parser, fallback, and service to avoid single-path drift.
- [Phase 06]: Resolve learningPathId for every ranked onboarding slug in the backend before returning API data.
- [Phase 06]: Allow parser degradation to 1-3 valid ranked items, but require fallback to always supply three deterministic recommendations.
...
Last session: 2026-03-26T04:36:15.364Z
